...
/**
 * Tests basic create/read/commit/delete functionality of ConsumerManager. This
 * only exercises the functionality for binary data because it uses a mock
 * consumer that only works with byte[] data.
 */
public class ConsumerManagerTest {
  ...
  public void setUp() throws RestConfigException {
    ...
    // This setting supports the testConsumerOverrides test. It is otherwise benign
    // and should
  ...
  private ConsumerConnector expectCreate(
      Map<String, List<Map<Integer, List<ConsumerRecord<byte[], byte[]>>>>> schedules, boolean allowMissingSchedule,
      String requestedId) {
    ConsumerConnector consumer = new MockConsumerConnector(config.getTime(), "testclient", schedules,
        Integer.parseInt(KafkaRestConfig.CONSUMER_ITERATOR_TIMEOUT_MS_DEFAULT), allowMissingSchedule);
    capturedConsumerConfig = Capture.newInstance();
    EasyMock.expect(consumerFactory.createConsumer(EasyMock.capture(capturedConsumerConfig))).andReturn(consumer);
    return consumer;
  }

  // Expect a Kafka consumer to be created, but return it with no data in its
  // queue. Used to test
  ...
  public void testConsumerOverrides() {
    ConsumerConnector consumer = new MockConsumerConnector(config.getTime(), "testclient", null,
        Integer.parseInt(KafkaRestConfig.CONSUMER_ITERATOR_TIMEOUT_MS_DEFAULT), true);
    ...
    // The exclude.internal.topics setting is overridden via the constructor when
    // the
  ...
  public void testConsumerNormalOps() throws Exception {
    ...
    // Mock consumer doesn't handle offsets, so we just check we get some output for
    // the
    // right partitions
  ...
  /**
   * Response should return no sooner than
  ...
  public void testConsumerTimeoutMsMsAndMinBytes() throws Exception {
    ...
    assertEquals("Records returned not as expected", Arrays.asList(referenceRecords.get(0)), actualRecords);
  ...
  /**
   * Response should return no sooner than the overridden
  ...
  public void testConsumerRequestTimeoutMsIsOverriddablePerConsumer() throws Exception {
    ...
    ConsumerInstanceConfig consumerConfig = new ConsumerInstanceConfig(null, null, EmbeddedFormat.BINARY.name(), null,
        null, null, overriddenRequestTimeMs);
  ...
  public void testConsumerMaxBytesResponse() throws Exception {
    // Tests that when there are more records available than the max bytes to be
    // included in the
    // response, not all of it is returned.
    final List<ConsumerRecord<byte[], byte[]>> referenceRecords = Arrays.<ConsumerRecord<byte[], byte[]>>asList(
    ...
    readTopic(cid, new ConsumerReadCallback<byte[], byte[]>() {
      ...
      public void onCompletion(List<? extends ConsumerRecord<byte[], byte[]>> records, RestException e) {
        sawCallback = true;
        actualException = e;
        // Should only see the first two messages since the third pushes us over the
        // limit.
  ...
  public void testIDOverridesName() {
    // We should remain compatible with the original use of consumer IDs, even if it
    // shouldn't
    // really be used. Specifying any ID should override any naming to ensure the
    // same behavior
  ...
  public void testDuplicateConsumerName() {
    ...
    consumerManager.createConsumer(groupName,
  ...
  public void testMultipleTopicSubscriptionsFail() throws Exception {
    ...
    assertEquals("Callback Exception should be for already subscribed consumer",
        Errors.CONSUMER_ALREADY_SUBSCRIBED_ERROR_CODE, ((RestException) actualException).getErrorCode());
  ...
  public void testBackoffMsControlsPollCalls() throws Exception {
    ...
    // backoff is 250
    Thread.sleep(100);
    // backoff should be in place right now. the read task should be delayed and
    // re-ran until the max.bytes or timeout is hit
  ...
  public void testReadInvalidTopicFails() throws Exception, ExecutionException {
    ...
    String instanceId = consumerManager.createConsumer(groupName, new ConsumerInstanceConfig(EmbeddedFormat.BINARY));
  ...
  public void testConsumerExceptions() throws Exception {
    // We should be able to handle an exception thrown by the consumer, then issue
    // another
  ...
  /**
   * We need to wait for topic reads to finish. We can't rely on the returned
   * Future because one whole read task is spanned throughout multiple Runnables.
   * They are created and scheduled on the executor until the whole task is
   * finished
   */
  ...
  private void readTopic(final String cid, String topic, long maxBytes, final ConsumerReadCallback callback)
      throws Exception {
  ...
  private List<ConsumerRecord<byte[], byte[]>> referenceRecords(int count) {
    ...
    for (int i = 0; i < count; i++) {
      records.add(new BinaryConsumerRecord(topicName, ("k" + (i + 1)).getBytes(), ("v" + (i + 1)).getBytes(), i, 0));
    }
    return records;
  }
  ...
  private void readFromDefault(String cid) throws InterruptedException, ExecutionException {
    ...
    actualException = null;
    actualRecords = null;
    consumerManager.readTopic(groupName, cid, topicName, BinaryConsumerState.class, Long.MAX_VALUE,
