...
/**
 * Tracks all the state for a consumer. This class is abstract in order to
 * support multiple serialization formats. Implementations must provide decoders
 * and a method to convert {@code KafkaMessageAndMetadata<K,V>} values to
 * ConsumerRecords that can be returned to the client (including translation if
 * the decoded Kafka consumer type and ConsumerRecord types differ).
 */
public abstract class ConsumerState<KafkaKeyT, KafkaValueT, ClientKeyT, ClientValueT> {
  ...
  public volatile long expiration;
  // A read/write lock on the ConsumerState allows concurrent readTopic calls, but
  // allows
  // commitOffsets to safely lock the entire state in order to get correct
  // information about all
  // the topic/stream's current offset state. All operations on individual
  // TopicStates must be
  // synchronized at that level as well (so, e.g., readTopic may modify a single
  // TopicState, but
  ...
  /**
   * Converts a MessageAndMetadata using the Kafka decoder types into a
   * ConsumerRecord using the client's requested types. While doing so, computes
   * the approximate size of the message in bytes, which is used to track the
   * approximate total payload size for consumer read responses to determine when
  ...
  /**
   * Start a read on the given topic, enabling a read lock on this ConsumerState
   * and a full lock on the ConsumerTopicState.
   */
  ...
  /**
   * Finish a read request, releasing the lock on the ConsumerTopicState and the
   * read lock on this ConsumerState.
   */
  ...
  public void close() {
    ...
    try {
      consumer.shutdown();
      // Marks this state entry as no longer valid because the consumer group is being
      // destroyed.
      consumer = null;
      topics = null;
    } finally {
  ...
  public ConsumerTopicState<KafkaKeyT, KafkaValueT, ClientKeyT, ClientValueT> getOrCreateTopicState(String topic) {
    ...
    try {
      ...
      ConsumerTopicState<KafkaKeyT, KafkaValueT, ClientKeyT, ClientValueT> state = topics.get(topic);
      if (state != null) {
        return state;
      }
    } finally {
    ...
    try {
      ...
      Map<String, List<KafkaStream<KafkaKeyT, KafkaValueT>>> streamsByTopic = consumer
          .createMessageStreams(subscriptions, getKeyDecoder(), getValueDecoder());
  ...
  /**
   * Gets a list of TopicPartitionOffsets describing the current state of consumer
   * offsets, possibly updating the commmitted offset record. This method is not
   * synchronized.
   *
   * @param updateCommitOffsets if true, updates committed offsets to be the same
   *                            as the consumed offsets.
   */
  private List<TopicPartitionOffset> getOffsets(boolean updateCommitOffsets) {
    ...
    for (Map.Entry<String, ConsumerTopicState<KafkaKeyT, KafkaValueT, ClientKeyT, ClientValueT>> entry : topics
        .entrySet()) {
      ...
      try {
        for (Map.Entry<Integer, Long> partEntry : state.getConsumedOffsets().entrySet()) {
          ...
          result.add(new TopicPartitionOffset(entry.getKey(), partition, offset,
